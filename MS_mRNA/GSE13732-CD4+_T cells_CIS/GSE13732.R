...
rownames(sampleInfo) <- colnames(corMatrix)
pheatmap(corMatrix,
         annotation_col=sampleInfo, main = "Normalized data")    
...
design
colnames(design) <- c("CIS","HC") ## rename the columns to select them easly later 
...
ids_of_interest <-  filter(full_results,ID %in% my_genes) %>% 
  pull(ID)
