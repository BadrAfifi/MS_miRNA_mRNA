...
log_eset <- log2(exprs(gse1))
plot(density(unlist(log_eset)))

#       ---[1- histogram]
cols=seq(1:length(colnames(gse1)))
hist(log_eset ,main = "Histogram Log Normalized data ",col=cols, prob = TRUE)                               # Histogram and density
lines(density(log_eset), col = "black")
legend(12,0.9, colnames(log_eset),col=cols,lty=1, lwd=2,cex=0.5)

#     ---[2- box plots]
boxplot(log_eset, main = "Box Plot Log Normalized data",col=seq(1:23))
...
sampleInfo$`disease state:ch1` <- c(rep("HC", 8),rep("ALS", 8),rep("MS", 8))#Optionally, rename to shorter sample names
...
corMatrix <- cor(log_eset,use="c") ## argument use="c" stops an error if there are any missing data points
rownames(sampleInfo) <- colnames(corMatrix)
pheatmap(corMatrix,
         annotation_col=sampleInfo, main = "Log Normalized data",fontsize_row = 3,fontsize_col = 3)    

#     ---[5- rincipal Components Analysis (PCA)]

#**** A complementary approach is to use Principal Components Analysis (PCA).
## MAKE SURE TO TRANSPOSE THE EXPRESSION MATRIX
pca <- prcomp(t(log_eset),)
## Join the PCs to the sample information
cbind(sampleInfo, pca$x) %>% 
  ggplot(aes(x = PC1, y=PC2, col=Condition,main = "Log Normalized data",
             label=paste("Condition", Condition))) + geom_point()

# 5.2 -Multidimensional scaling plot of distances between gene expression profiles
plotMDS(log_eset, labels = sampleInfo[,"Condition"],
        gene.selection = "common", main = "Log Normalized data")
...
design
colnames(design) <- c("ALS","HC","MS") ## rename the columns to select them easly later 
...
aw <- arrayWeights(log_eset,design)
aw
...
fit <- lmFit(log_eset, design,weights = aw)
